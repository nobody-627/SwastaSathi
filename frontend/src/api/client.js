...
export const api = {
  ...
  medications: {
    ...
    markTaken: (id, time) => api.post(`/medications/${id}/taken`, { time }),
  },
  activity: {
    summary: (date) => api.get(`/activity/summary${date ? `?date=${date}` : ''}`),
    ingest: (payload) => api.post('/activity/ingest', payload),
    setGoals: (payload) => api.put('/activity/goals', payload),
  },
};
...
export function createVitalsSocket(onMessage, onError) {
  const wsUrl =
    (import.meta.env.VITE_WS_URL || "ws://localhost:3001") + "/ws/vitals";
  let ws = null;
