...
const generateReading = (baseline, mode) => {
  ...
  const modes = {
    cardiac: {
      hr: Math.round(130 + noise(15)),
      spo2: Math.round((93 + noise(2)) * 10) / 10,
      temp: Math.round((37.1 + noise(0.3)) * 10) / 10,
      hrv: Math.round(10 + noise(4)),
    },
    hypoxic: {
      hr: Math.round(120 + noise(12)),
      spo2: Math.round((87 + noise(3)) * 10) / 10,
      temp: Math.round((37.3 + noise(0.2)) * 10) / 10,
      hrv: Math.round(11 + noise(3)),
    },
    stress: {
      hr: Math.round(105 + noise(10)),
      spo2: Math.round((94 + noise(2)) * 10) / 10,
      temp: Math.round((37.2 + noise(0.3)) * 10) / 10,
      hrv: Math.round(17 + noise(5)),
    },
  };
...
wss.on("connection", (ws, req) => {
  ...
  ws.send(
    JSON.stringify({
      type: "connected",
      clientId,
      message: "SwasthSathi WebSocket connected",
    }),
  );
